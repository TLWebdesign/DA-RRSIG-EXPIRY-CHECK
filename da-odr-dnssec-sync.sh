...
#*
#* @brief       Dit script verzorgt de registratie van de DNSSEC Keys bij ODR
#*
#* @details     Er wordt gekeken of er DNSSEC keys zijn op de server.
#*              Deze worden vergeleken met die van ODR. Als deze anders zijn worden ze aangepast.
...
ODR_CREDENTIALS["tladmin,public"]="abcd"
ODR_CREDENTIALS["tladmin,private"]="1234"
ODR_CREDENTIALS["hein,public"]="efgh"
ODR_CREDENTIALS["hein,private"]="5678"

SLD=`echo ${DOMAIN} | cut -d'.' -f1`
TLD=`echo ${DOMAIN} | cut -d'.' -f2`

echo "=============$(date)============"
echo "We gaan DNSSEC sync met ODR uitvoeren voor $DOMAIN"


curl -s "https://api.opendomainregistry.com/command.php?apiuser=${OXXA_USER}&apipassword=${OXXA_PASS}&command=dnssec_info&sld=${SLD}&tld=${TLD}" > response.xml
